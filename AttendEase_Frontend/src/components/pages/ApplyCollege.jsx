...
const ApplyCollege = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await apiClient.post("/api/colleges/collegeApplication", { collegeName: clgName, authorityName, authorityRole: role, officialEmail: email });
            toast.success('Your Application Submitted Successfully.');
            localStorage.setItem('Application', 'submitted');
            setIsApplicationSubmitted(true);
        } catch (error) {
            toast.error("There is an Error While Submitting You Application. please try again later!");
        }
        setDisable(false);
        setBtntxt('Submit');
    }
    ...
    return (
        <div className="ApplyCollege">
            <form className={isApplicationSubmitted ? 'hide' : 'wrapper'} onSubmit={handleSubmit}>
                <div className="logo">
            ...
            <h2 className={isApplicationSubmitted ? 'submitted' : 'hide'}>Application Already Submitted from your device ✅</h2>
